...
public class ClockHand_Second implements GraphicComponent {
	...
	public ClockHand_Second(Applet clock, CurrentTime timeFetcher) {
		this.screen = clock;
	...
	public void step() {
		...
		radius = Math.min(screen.getWidth()/2, screen.getHeight()/2);
		//Calculate angle based on proportion of hours to proportion of angle converted to radians
		angle = ((time.getTime().getSeconds()/60.0)*2*Math.PI); //in radians
