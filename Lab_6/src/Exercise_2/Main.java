...
public class Main {
	...
	private ArrayList<Component> getComponents() {
		...
		paneTop.add(makeArgumentPanel(new String[] { "X", "Y", "Width", "Height" }, "Grid Position"));
		paneTop.add(makeArgumentPanel(new String[] { "Top", "Left", "Bottom", "Right" }, "External Insets"));
		paneTop.add(makeArgumentPanel(new String[] { "Width", "Height" }, "Size Padding"));
		...
		paneCenter.add(makeArgumentPanel(new String[] { "X", "Y" }, "Weight"));
		paneCenter.add(makeRadioButtonPanel(new String[] { "NW", "N", "NE", "W", "C", "E", "SW", "S", "SE" }, 3, 3, 4,
				"Anchor"));
		components.add(paneCenter);

		components
				.add(makeRadioButtonPanel(new String[] { "None", "Horizontal", "Vertical", "Both" }, 1, 4, 1, "Fill"));
	...
	private JPanel makeArgumentPanel(String[] labels, String title) {
		...
		newPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), title));
	...
	/**
	 * Build a panel of radio button choices with single checkbox group
	 * @param labels Names for buttons
	 * @param rows	Layout row number
	 * @param columns Layout column number
	 * @param defaultIndex Default choice's index
